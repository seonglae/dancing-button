...
class DancingButton {
  ...
  constructor(svg: SVGElement, options: any = {}) {
    svg.style.overflow = 'visible'
    this.id = String(DancingButton.count++)
  ...
  createPoint(x: number, y: number): DaningPoint {
    return {
      x: x,
      y: y,
      ox: x,
  ...
  initOrigins() {
    ...
    for (let layerIndex = 0; layerIndex < this.layers.length; layerIndex++) {
      const layer = this.layers[layerIndex]
      const points: Array<DaningPoint> = []
      for (let x = ~~(this.height / 2); x < this.width - ~~(this.height / 2); x += this.gap)
        points.push(this.createPoint(x + this.margin, this.margin - layerIndex))

      for (let alpha = ~~(this.height * 1.25); alpha >= 0; alpha -= this.gap) {
        const angle = (Math.PI / ~~(this.height * 1.25)) * alpha
        points.push(
          this.createPoint(
            (Math.sin(angle) * this.height) / 2 + this.margin + this.width - this.height / 2 + layerIndex,
            (Math.cos(angle) * this.height) / 2 + this.margin + this.height / 2 - layerIndex
          )
        )
      }
      for (let x = this.width - ~~(this.height / 2) - 1; x >= ~~(this.height / 2); x -= this.gap)
        points.push(this.createPoint(x + this.margin, this.margin + this.height - layerIndex))

      for (let alpha = 0; alpha <= ~~(this.height * 1.25); alpha += this.gap) {
        const angle = (Math.PI / ~~(this.height * 1.25)) * alpha
        points.push(
          this.createPoint(
            this.height - (Math.sin(angle) * this.height) / 2 + this.margin - this.height / 2 - layerIndex,
            (Math.cos(angle) * this.height) / 2 + this.margin + this.height / 2 - layerIndex
          )
        )
      }
